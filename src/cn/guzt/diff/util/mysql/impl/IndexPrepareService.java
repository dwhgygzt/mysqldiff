...
@DiffPrepareMarker
public class IndexPrepareService extends AbstractPrepare {

    public IndexPrepareService() {
        super(DbObjType.INDEX);
    }

    @Override
    public List<JSONObject> getObjList(MysqlMapper mapper  , DbProperties db) {
        return mapper.getIndexs(db.getDbName());
    }

    @Override
    public void setRowToMap(List<JSONObject>  originalRow, Map<String, JSONObject> targetMapRow, List<String> targetRowKey) {
        CodeUtil.setRowToMap(originalRow, targetMapRow, targetRowKey, Constant.INDEX_KEY_COLUMN);
    }

    @Override
    public List<JSONObject> diffCompare(DiffPrepareBean original, DiffPrepareBean target) {
        return CodeUtil.diffCompare(original,target,Constant.INDEX);
    }
}
